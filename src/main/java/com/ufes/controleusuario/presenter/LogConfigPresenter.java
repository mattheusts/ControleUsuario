...
/**
 * LogConfigPresenter - Lógica para configuração de logs
...
public class LogConfigPresenter {
    ...
    private void loadCurrentConfig() {
        // Get current format from logger
        if (logger instanceof LoggerService) {
            String currentFormat = ((LoggerService) logger).getFormat();
            view.setCurrentFormat(currentFormat);
        } else {
            view.setCurrentFormat("CSV"); // Default
        }
    }
    ...
    private void onSave(ActionEvent e) {
        ...
        try {
            ...
            if (logger instanceof LoggerService) {
                ((LoggerService) logger).setFormat(selectedFormat);
            }
