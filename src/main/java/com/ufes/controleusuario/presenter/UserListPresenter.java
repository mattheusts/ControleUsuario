...
/**
 * UserListPresenter - Lógica para listagem e manutenção de usuários
...
public class UserListPresenter {
    ...
    private void configurePermissions() {
        // Only first admin can promote/demote
        if (!isFirstAdmin) {
    ...
    private void onTableSelection(ListSelectionEvent e) {
        ...
        if (isFirstAdmin) {
            // First admin can promote regular users, demote other admins (not self)
        ...
        // Delete: admins can delete users, but not themselves and regular admins can't delete other admins
        boolean canDelete = !isSelectedSelf;
        if (!isFirstAdmin && "ADMIN".equals(profile)) {
            canDelete = false; // Regular admin can't delete other admins
        }
        if (isSelectedFirstAdmin) {
            canDelete = false; // No one can delete first admin
        }
    ...
    private void onDemote(ActionEvent e) {
        ...
        // Cannot demote first admin or self
